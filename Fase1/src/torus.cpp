...
std::vector<NormalTexPoint> Torus::draw() {
	...
	for (int i = 0; i < slices; i++) {
		...
		for (int j = 0; j < stacks; j++) {
			...
			/*points.push_back(NormalTexPoint(p1,v1,(i*1.0)/slices, (j*1.0)/stacks));
			points.push_back(NormalTexPoint(p3,v3,((i+1)*1.0)/slices, (j*1.0)/stacks));
			points.push_back(NormalTexPoint(p2,v2,(i*1.0)/slices, ((j+1)*1.0)/stacks));

			points.push_back(NormalTexPoint(p2,v2,(i*1.0)/slices, ((j+1)*1.0)/stacks));
			points.push_back(NormalTexPoint(p3,v3,((i+1)*1.0)/slices, (j*1.0)/stacks));
			points.push_back(NormalTexPoint(p4,v4,((i+1)*1.0)/slices, ((j+1)*1.0)/stacks));*/
